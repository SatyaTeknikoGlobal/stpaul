...
// echo 'here'; die;
    ...
    public static function rank($user_id , $exam_id)
    {
     $ranks = Answer::all();
     $correct_ans = 0;
     $skipped_ans = 0;
   ...
   //          $correct_ans += $rank->status;              
   //          }

   //     if($rank->option == '')
   //     {
   //         $skipped_ans += $rank->option;
   //     }

   // }

     $correct_ans = Answer::where('user_id',$user_id)->where('exam_id',$exam_id)->where('status',1)->count();
     $wrong_ans = Answer::where('user_id',$user_id)->where('exam_id',$exam_id)->where('status',0)->count();


     $exams = Exam::where('id',$exam_id)->first();
     $exam_id =  $exams->id;                 
     $total_questions = $exams->no_of_questions; 

     $skipped_ans = $total_questions - ($correct_ans + $wrong_ans); 

     $time = AttemptExam::where(['status' => 1, 'user_id'=> $user_id, 'exam_id' => $exam_id])->get()->first();

     $start_time =  $time->start_time;
     $end_time =  $time->end_time;


     $st_num = explode(" ", $start_time);
     $en_nums = explode(" ",$end_time);


     $initial_time = strtotime($st_num[1]); 
     $final_time = strtotime($en_nums[1]);
     ...
     $exams = Exam::where(["id"=>$exam_id])->first();
     if(!empty($exams)){
        $positive_marks = $exams->marks_per_question;
        $negative_marks = $exams->negetive_mark;
    }

    $posi_mark = $correct_ans * $positive_marks;
    $neg_mark = $wrong_ans * $negative_marks;
    $marks = $posi_mark - $neg_mark;
    ...
    $time_arr  = [];
    foreach($get_rank as $grank)
    {

     $count_rank = $grank->time_taken;
     $time_arr[] = $count_rank; 
 ...
 for($i = 0; $i < $time_length; $i++ )
 {
    ...
    }elseif ($time_arr[$i] != $time_arr[$i-1])
    {
     $prev_rank = $rank;
     ...
     $time_arr[$i].$prev_rank;
 }

 $final_rank= $rank++;
}

DB::table('ranking')->where(['user_id'=> $user_id, 'exam_id' => $exam_id])->update(['rank' => $final_rank]);
