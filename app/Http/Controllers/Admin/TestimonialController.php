...
Class TestimonialController extends Controller
{
   ...
   public function get_testimonial(Request $request){
    ...
    ->editColumn('id', function(Testimonial $data) {

        return  $data->id;
    })
    ->editColumn('name', function(Testimonial $data) {
        $user = User::where('id',$data->user_name)->first();
        return  $user->name ?? '';
    })
    ->editColumn('text', function(Testimonial $data) {
        $text = mb_strlen(strip_tags($data->text),'utf-8') > 50 ? mb_substr(strip_tags($data->text),0,50,'utf-8').'...' : strip_tags($data->text);
        return  $text;
    })
    ...
    ->editColumn('status', function(Testimonial $data) {
     $sta = '';
     $sta1 ='';
     if($data->status == 1){
        $sta1 = 'selected';
    ...
    ->editColumn('created_at', function(Testimonial $data) {
    ...
    ->addColumn('action', function(Testimonial $data) {
      ...
      $BackUrl = $routeName.'/testimonials';


      return '<a title="Edit" href="' . route($routeName.'.testimonials.edit',$data->id.'?back_url='.$BackUrl) . '"><i class="fa fa-edit">Edit</i></a>&nbsp;&nbsp;&nbsp;';
  })
...
public function add(Request $request){
    ...
    if($request->method() == 'POST' || $request->method() == 'post'){
        ...
        if ($createdCat) {
            $alert_msg = 'Testimonials has been added successfully.';
            if(is_numeric($id) && $id > 0){
                $alert_msg = 'Testimonials has been updated successfully.';
            }
    ...
    $page_heading = 'Add Testimonials';

      if(is_numeric($id) && $id > 0){
        $page_heading = 'Update Testimonials';
    }  
...
public function change_status(Request $request){
  ...
  $testimonial = Testimonial::where('id',$test_id)->first();
  if(!empty($testimonial)){
     Testimonial::where('id',$test_id)->update(['status'=>$status]);
